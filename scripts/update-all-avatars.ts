...
const getRandomAvatar = () => {
  const avatar = avatarFiles[Math.floor(Math.random() * avatarFiles.length)];
...
async function updateAllAvatars() {
  try {
    ...
    // Update demo accounts with specific avatars
    const demoAvatars: Record<string, string> = {
      'admin@example.com': '/avatars/1a270860bac2c66b434968a3047822e3.webp',
      'user@example.com': '/avatars/2b04cc0b930f82afe6c38d3209dcbdfd.webp',
      'provider@example.com': '/avatars/1c9a4dd0bbd964e3eecbd40caf3b7e37.webp',
    };
    ...
    for (const profile of profiles) {
      ...
      if (email && demoAvatars[email]) {
        await prisma.profile.update({
          where: { id: profile.id },
          data: { avatarUrl: demoAvatars[email] },
        });
        console.log(`✅ Updated demo account: ${email}`);
        updatedCount++;
      } else if (!profile.avatarUrl || profile.avatarUrl === '') {
        // Assign random avatar to profiles without avatars
        const randomAvatar = getRandomAvatar();
        await prisma.profile.update({
          where: { id: profile.id },
          data: { avatarUrl: randomAvatar },
        });
        console.log(`✅ Updated profile: ${email || profile.id} with ${randomAvatar}`);
        updatedCount++;
      } else {
        console.log(`⏭️  Skipped profile (already has avatar): ${email || profile.id}`);
        skippedCount++;
      }
    }
