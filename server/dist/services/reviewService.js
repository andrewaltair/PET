...
class ReviewService {
    ...
    static async createReview(userId, reviewData) {
        ...
        const result = await database_1.prisma.$transaction(async (tx) => {
            ...
            if (booking.status !== petservice_marketplace_shared_types_1.BookingStatus.COMPLETED) {
                throw new Error('Can only review completed bookings');
            }
            if (booking.ownerId !== userId) {
                throw new Error('Only the booking owner can leave a review');
            }
            if (booking.owner.role !== petservice_marketplace_shared_types_1.UserRole.OWNER) {
     ...
     */
    static async canReviewBooking(userId, bookingId) {
        ...
        return (booking.status === petservice_marketplace_shared_types_1.BookingStatus.COMPLETED &&
            booking.ownerId === userId &&
            booking.owner.role === petservice_marketplace_shared_types_1.UserRole.OWNER &&
            !booking.review);
    }
    /**
     * Recalculate average rating for a service
     */
    static async recalculateServiceRating(tx, serviceId) {
     ...
     */
    static async recalculateProviderRating(tx, providerId) {
